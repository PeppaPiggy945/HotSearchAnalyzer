...
class WeChatCrawler(BaseCrawler):
    """微信热文榜爬虫（通过 tophub.today 获取）"""

    API_URL = "https://tophub.today/node-items-by-date"
    NODE_ID = "5"  # 微信热文榜的 nodeid
    ...
    def fetch(self) -> HotSearchResult | None:
        """获取微信热文榜"""
        self.logger.info(f"正在爬取{self.platform_name}热文榜...")
        ...
        try:
            ...
            self.logger.error(f"{self.platform_name}热文榜爬取失败: {e}")
    ...
    def _parse(self, data: dict) -> List[HotSearchItem]:
        ...
        for i, item in enumerate(raw_items):
            try:
                ...
                self.logger.debug(f"解析微信热文条目失败: {e}")
                continue
